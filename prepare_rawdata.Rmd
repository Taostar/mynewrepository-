...
fun_buscat_perc = function (base.dt, varnames)
                  {
                    ...
                    for (atr in varnames) {
                       ...
                       atr.dt = dt[,sapply(atr.levels, 
                                           function (x) {lev.count = .SD[get(atr) == x, .N]; 
                                                         total.count = .N; 
                                                         .(lev.count/total.count)}), keyby = CATEGORY_CD]
                       
                       setnames(atr.dt, atr.levels, atr.names)
                      
                       atr.perc.buscat = merge(atr.perc.buscat, atr.dt, by = "CATEGORY_CD")
                       # if (i == 1){
                       #   atr.perc.buscat = atr.dt
                       # } else {
                       # atr.perc.buscat = cbind(atr.perc.buscat, atr.dt[,-1])}
                       # i= i + 1
                    }
